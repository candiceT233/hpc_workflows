#!/usr/bin/env bash
#SBATCH --job-name=smrna_hq_dl18
#SBATCH --partition=compute
#SBATCH --output=/mnt/common/mtang11/hpc_workflows/runs/nf-core_smrnaseq/multinode_18node_hq_datalife/logs/slurm_%j.out
#SBATCH --error=/mnt/common/mtang11/hpc_workflows/runs/nf-core_smrnaseq/multinode_18node_hq_datalife/logs/slurm_%j.err
...
RUNDIR=$WORKFLOW_ROOT/runs/nf-core_smrnaseq/multinode_18node_hq_datalife
TAG=hq_datalife_18node
LOG=$RUNDIR/logs
...
export DATALIFE_FILE_PATTERNS='*.fastq,*.fastq.gz,*.fq,*.fq.gz,*.bam,*.sam,*.fa,*.fasta,*.gtf,*.bed,*.gff,*.tsv,*.csv,*.txt,*.mirna,*.hairpin'
...
nextflow run $WORKFLOW_ROOT/repos/nf-core_smrnaseq/main.nf \
    -profile test,conda \
