#!/usr/bin/env bash
#SBATCH --job-name=methyl_hq_pl
#SBATCH --partition=compute
#SBATCH --output=/mnt/common/mtang11/hpc_workflows/runs/nf-core_methylseq/multinode_2node_hq_plain/logs/slurm_%j.out
#SBATCH --error=/mnt/common/mtang11/hpc_workflows/runs/nf-core_methylseq/multinode_2node_hq_plain/logs/slurm_%j.err
...
RUNDIR=$WORKFLOW_ROOT/runs/nf-core_methylseq/multinode_2node_hq_plain
TAG=hq_plain_2node
LOG=$RUNDIR/logs
...
mkdir -p "$NXF_WORK" "$LOG" "$RUNDIR/outputs_${TAG}" "$DATALIFE_OUTPUT_PATH"
...
nextflow run $WORKFLOW_ROOT/repos/nf-core_methylseq/main.nf \
    -profile test,conda \
    -c $WORKFLOW_ROOT/runs/nf-core_shared/hq.config \
