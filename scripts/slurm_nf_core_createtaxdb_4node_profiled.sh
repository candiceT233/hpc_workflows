#!/usr/bin/env bash
#SBATCH --job-name=createtaxdb-prof
#SBATCH --exclusive
...
#SBATCH --output=runs/nf-core_createtaxdb/slurm-%j-4node-profiled.out
#SBATCH --error=runs/nf-core_createtaxdb/slurm-%j-4node-profiled.err
...
for required in "$NF_ENV/bin/nextflow" "$DATALIFE_LIB" "$DARSHAN_LIB" "$DARSHAN_PARSER" "$ROOT/scripts/profile_tree_io.py" "$CREATETAXDB_INPUT" "$CREATETAXDB_ACCESSION2TAXID" "$CREATETAXDB_PROT2TAXID" "$CREATETAXDB_NODESDMP" "$CREATETAXDB_NAMESDMP"; do
  if [ ! -s "$required" ] && [ ! -x "$required" ]; then
    echo "ERROR: missing required nf-core/createtaxdb profiling artifact: $required" >&2
    exit 1
  fi
done
...
EOF
  ...
  if ! find "$outdir" -type f \( -name '*.fmi' -o -name '*.k2d' -o -name '*.dmnd' -o -name '*.mmi' -o -name '*.sbt.zip' \) -size +0 | grep -q .; then
    echo "ERROR: ${mode} replica $replica has no non-empty classifier database artifacts" >&2
    exit 1
  fi
  ...
  if [ "$mode" = "datalife" ]; then
    env DATALIFE_OUTPUT_PATH="$trace_dir" \
      DATALIFE_FILE_PATTERNS="*.fmi,*.k2d,*.dmnd,*.mmi,*.sbt.zip,*.fa,*.fna,*.fasta,*.gz,*.map,*.dmp,*.tsv,*.csv,*.txt,*.json,*.log,*.yaml,*.yml" \
      LD_PRELOAD="$DATALIFE_LIB" \
      "$NF_ENV/bin/python" "$ROOT/scripts/profile_tree_io.py" \
        --root "$outdir" \
        --manifest "$rep_dir/profiled-output-digests.json" \
        --patterns '*.fmi' '*.k2d' '*.dmnd' '*.mmi' '*.sbt.zip' '*.fa' '*.fna' '*.fasta' '*.gz' '*.map' '*.dmp' '*.tsv' '*.csv' '*.txt' '*.json' '*.log' '*.yaml' '*.yml'
  ...
  echo "nf-core/createtaxdb ${mode} replica $replica native Nextflow workflow completed with $output_count non-empty output files"
}
...
echo "nf-core/createtaxdb native 4-node profiled Nextflow workflow completed across ${node_count} nodes"
