#!/usr/bin/env bash
#SBATCH --job-name=deepvariant-prof
#SBATCH --exclusive
...
#SBATCH --output=runs/nf-core_deepvariant/slurm-%j-4node-profiled.out
#SBATCH --error=runs/nf-core_deepvariant/slurm-%j-4node-profiled.err
...
for required in "$NF_ENV/bin/nextflow" "$BAM" "$BAM.bai" "$BED" "$FASTA" "$DATALIFE_LIB" "$DARSHAN_LIB" "$DARSHAN_PARSER" "$ROOT/scripts/profile_tree_io.py"; do
  if [ ! -s "$required" ] && [ ! -x "$required" ]; then
    echo "ERROR: missing required nf-core/deepvariant profiling artifact: $required" >&2
    exit 1
  fi
done
...
run_replica() {
  ...
  local podman_base="/tmp/pdm-deepvariant-${SLURM_JOB_ID:-$$}-${mode}-${replica}"

  mkdir -p "$outdir" "$workdir" "$tmp_root" "$pkgs_root" "$cache_root" "$trace_dir" "$rep_dir/podman-bin" "$podman_base"/{xdg,root,run,tmp}
...
EOF
  ...
  if ! find "$outdir/Documentation" -type f -name 'pipeline_report.html' -size +1000 | grep -q .; then
    echo "ERROR: ${mode} replica $replica missing non-trivial DeepVariant pipeline HTML report" >&2
    exit 1
  fi
  if ! find "$outdir" -maxdepth 1 -type f -name '*.vcf' -size +0 | grep -q .; then
    echo "ERROR: ${mode} replica $replica missing non-empty DeepVariant VCF output" >&2
    exit 1
  fi
  ...
  if [ "$mode" = "datalife" ]; then
    env DATALIFE_OUTPUT_PATH="$trace_dir" \
      DATALIFE_FILE_PATTERNS="*.bam,*.bai,*.bed,*.vcf,*.vcf.gz,*.gvcf,*.gvcf.gz,*.tbi,*.fa,*.fasta,*.fai,*.txt,*.tsv,*.csv,*.html,*.log,*.yaml,*.yml" \
      LD_PRELOAD="$DATALIFE_LIB" \
      "$NF_ENV/bin/python" "$ROOT/scripts/profile_tree_io.py" \
        --root "$outdir" \
        --manifest "$rep_dir/profiled-output-digests.json" \
        --patterns '*.bam' '*.bai' '*.bed' '*.vcf' '*.vcf.gz' '*.gvcf' '*.gvcf.gz' '*.tbi' '*.fa' '*.fasta' '*.fai' '*.txt' '*.tsv' '*.csv' '*.html' '*.log' '*.yaml' '*.yml'
  ...
  echo "nf-core/deepvariant ${mode} replica $replica native Nextflow workflow completed with $output_count non-empty output files"
}
...
echo "nf-core/deepvariant native 4-node profiled Nextflow workflow completed across ${node_count} nodes"
