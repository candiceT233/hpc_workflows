#!/usr/bin/env bash
#SBATCH --job-name=methylseq-prof
#SBATCH --exclusive
...
#SBATCH --output=runs/nf-core_methylseq/slurm-%j-4node-profiled.out
#SBATCH --error=runs/nf-core_methylseq/slurm-%j-4node-profiled.err
...
for required in "$NF_ENV/bin/nextflow" "$DATALIFE_LIB" "$DARSHAN_LIB" "$DARSHAN_PARSER" "$ROOT/scripts/profile_tree_io.py" "$INPUT" "$FASTA"; do
  if [ ! -s "$required" ] && [ ! -x "$required" ]; then
    echo "ERROR: missing required nf-core/methylseq profiling artifact: $required" >&2
    exit 1
  fi
done
...
EOF
  ...
  if ! find "$outdir/bismark" -type f -size +0 | grep -q .; then
    echo "ERROR: ${mode} replica $replica missing non-empty Bismark outputs" >&2
    exit 1
  fi
  if ! find "$outdir/bismark/methylation_calls" -type f -size +0 | grep -q .; then
    echo "ERROR: ${mode} replica $replica missing non-empty methylation-call outputs" >&2
    exit 1
  fi
  output_count="$(find "$outdir" -type f -size +0 | wc -l)"
  if [ "$output_count" -lt 50 ]; then
    echo "ERROR: ${mode} replica $replica too few non-empty methylseq outputs: $output_count" >&2
    exit 1
  fi
  ...
  if [ "$mode" = "datalife" ]; then
    env DATALIFE_OUTPUT_PATH="$trace_dir" \
      DATALIFE_FILE_PATTERNS="*.fastq.gz,*.fq.gz,*.bam,*.bai,*.bedGraph,*.cov,*.txt,*.tsv,*.csv,*.html,*.json,*.log,*.pdf,*.png,*.gz" \
      LD_PRELOAD="$DATALIFE_LIB" \
      "$NF_ENV/bin/python" "$ROOT/scripts/profile_tree_io.py" \
        --root "$outdir" \
        --manifest "$rep_dir/profiled-output-digests.json" \
        --patterns '*.fastq.gz' '*.fq.gz' '*.bam' '*.bai' '*.bedGraph' '*.cov' '*.txt' '*.tsv' '*.csv' '*.html' '*.json' '*.log' '*.pdf' '*.png' '*.gz'
  ...
  echo "nf-core/methylseq ${mode} replica $replica native Nextflow workflow completed with $output_count non-empty output files"
}
...
echo "nf-core/methylseq native 4-node profiled Nextflow workflow completed across ${node_count} nodes"
