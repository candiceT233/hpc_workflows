...
#SBATCH --output=runs/nf-core_ampliseq/slurm-%j-4node-profiled.out
#SBATCH --error=runs/nf-core_ampliseq/slurm-%j-4node-profiled.err
...
for required in "$NF_ENV/bin/nextflow" "$DATALIFE_LIB" "$DARSHAN_LIB" "$DARSHAN_PARSER" "$ROOT/scripts/profile_tree_io.py" \
  "$AMPLISEQ_SAMPLESHEET" "$AMPLISEQ_METADATA" "$REF_DIR/sbdi-gtdb.assignTaxonomy.fna.gz" \
  "$REF_DIR/sbdi-gtdb.addSpecies.fna.gz" "$REF_DIR/85_otus.fna" "$REF_DIR/85_otu_taxonomy.tax"; do
  if [ ! -s "$required" ] && [ ! -x "$required" ]; then
    echo "ERROR: missing required nf-core/ampliseq profiling artifact: $required" >&2
    exit 1
  fi
done
...
EOF
  ...
  if [ "$mode" = "datalife" ]; then
    env DATALIFE_OUTPUT_PATH="$trace_dir" \
      DATALIFE_FILE_PATTERNS="*.fastq.gz,*.qza,*.qzv,*.biom,*.tsv,*.txt,*.html,*.csv,*.json,*.log,*.yaml,*.yml,*.fasta,*.fa" \
      LD_PRELOAD="$DATALIFE_LIB" \
      "$NF_ENV/bin/python" "$ROOT/scripts/profile_tree_io.py" \
        --root "$outdir" \
        --manifest "$rep_dir/profiled-output-digests.json" \
        --patterns '*.fastq.gz' '*.qza' '*.qzv' '*.biom' '*.tsv' '*.txt' '*.html' '*.csv' '*.json' '*.log' '*.yaml' '*.yml' '*.fasta' '*.fa'
  ...
  echo "nf-core/ampliseq ${mode} replica $replica native Nextflow workflow completed with $output_count non-empty output files"
}
...
echo "nf-core/ampliseq native 4-node profiled Nextflow workflow completed across ${node_count} nodes"
