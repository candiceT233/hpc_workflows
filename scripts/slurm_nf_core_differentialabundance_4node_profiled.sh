#!/usr/bin/env bash
#SBATCH --job-name=diffabund-prof
#SBATCH --exclusive
...
#SBATCH --output=runs/nf-core_differentialabundance/slurm-%j-4node-profiled.out
#SBATCH --error=runs/nf-core_differentialabundance/slurm-%j-4node-profiled.err
...
RUN_ROOT="$ROOT/runs/nf-core_differentialabundance"
...
for required in "$NF_ENV/bin/nextflow" "$DATALIFE_LIB" "$DARSHAN_LIB" "$DARSHAN_PARSER" "$ROOT/scripts/profile_tree_io.py"; do
  if [ ! -s "$required" ] && [ ! -x "$required" ]; then
    echo "ERROR: missing required nf-core/differentialabundance profiling artifact: $required" >&2
    exit 1
  fi
done
...
run_replica() {
  ...
  local override="$rep_dir/differentialabundance_ares_override.config"
...
export NXF_OFFLINE=true
  ...
  if ! find "$outdir/report" -type f -name '*.html' -size +100000 | grep -q .; then
    echo "ERROR: ${mode} replica $replica missing non-trivial differentialabundance HTML report" >&2
    exit 1
  fi
  if ! find "$outdir/tables" -type f \( -name '*.tsv' -o -name '*.csv' \) -size +0 | grep -q .; then
    echo "ERROR: ${mode} replica $replica missing non-empty output tables" >&2
    exit 1
  fi
  ...
  if [ "$mode" = "datalife" ]; then
    env DATALIFE_OUTPUT_PATH="$trace_dir" \
      DATALIFE_FILE_PATTERNS="*.tsv,*.csv,*.txt,*.html,*.RData,*.rds,*.png,*.pdf,*.log,*.yaml,*.yml" \
      LD_PRELOAD="$DATALIFE_LIB" \
      "$NF_ENV/bin/python" "$ROOT/scripts/profile_tree_io.py" \
        --root "$outdir" \
        --manifest "$rep_dir/profiled-output-digests.json" \
        --patterns '*.tsv' '*.csv' '*.txt' '*.html' '*.RData' '*.rds' '*.png' '*.pdf' '*.log' '*.yaml' '*.yml'
  ...
  echo "nf-core/differentialabundance ${mode} replica $replica native Nextflow workflow completed with $output_count non-empty output files"
}
...
echo "nf-core/differentialabundance native 4-node profiled Nextflow workflow completed across ${node_count} nodes"
