#!/usr/bin/env bash
#SBATCH --job-name=smrnaseq-4node
#SBATCH --exclusive
...
#SBATCH --output=runs/nf-core_smrnaseq/slurm-%j-4node.out
#SBATCH --error=runs/nf-core_smrnaseq/slurm-%j-4node.err
...
for required in "$NF_ENV/bin/nextflow" "$INPUT"; do
  if [ ! -s "$required" ] && [ ! -x "$required" ]; then
    echo "ERROR: missing required smrnaseq artifact: $required" >&2
    exit 1
  fi
done
...
export NXF_OFFLINE=true
  ...
  echo "nf-core/smrnaseq replica $replica native Nextflow baseline completed with $output_count non-empty output files"
}
...
echo "nf-core/smrnaseq native 4-node Nextflow baseline completed across 4 nodes"
