#!/bin/bash
#SBATCH --job-name=pyflextrkr-4node
#SBATCH --exclusive
...
#SBATCH --output=runs/PyFLEXTRKR/slurm-%j-4node.out
#SBATCH --error=runs/PyFLEXTRKR/slurm-%j-4node.err
...
RUN_ROOT="$ROOT/runs/PyFLEXTRKR/4node"
...
scontrol show hostnames "$SLURM_JOB_NODELIST" > "$RUN_ROOT/hosts_${SLURM_JOB_ID:-manual}.txt"
NODE_COUNT="$(wc -l < "$RUN_ROOT/hosts_${SLURM_JOB_ID:-manual}.txt")"
...
echo "PyFLEXTRKR 4-node dask-mpi baseline completed with $OUTPUT_COUNT non-empty output files across $NODE_COUNT nodes"
