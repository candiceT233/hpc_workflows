...
#SBATCH --output=runs/nf-core_bacass/slurm-%j-4node.out
#SBATCH --error=runs/nf-core_bacass/slurm-%j-4node.err
...
for required in "$NF_ENV/bin/nextflow" "$INPUT_TSV" \
  "$FASTQ_DIR/SRR8482585_1.fastq.gz" "$FASTQ_DIR/SRR8482585_2.fastq.gz" \
  "$FASTQ_DIR/SRR10093029_1.fastq.gz" "$FASTQ_DIR/SRR10093029_2.fastq.gz"; do
  if [ ! -s "$required" ] && [ ! -x "$required" ]; then
    echo "ERROR: missing required nf-core/bacass artifact: $required" >&2
    exit 1
  fi
done
...
export NXF_OFFLINE=true
  ...
  echo "nf-core/bacass replica $replica native Nextflow baseline completed with $output_count non-empty output files"
}
...
echo "nf-core/bacass native 4-node Nextflow baseline completed across 4 nodes"
