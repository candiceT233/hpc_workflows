...
#SBATCH --output=runs/nf-core_quantms/slurm-%j-4node.out
#SBATCH --error=runs/nf-core_quantms/slurm-%j-4node.err
...
export NXF_OFFLINE=true
  ...
  if ! find "$outdir" -type f \( -name '*.mzTab' -o -name '*.mztab' -o -name '*msstats*.csv' -o -name '*triqler*.tsv' \) -size +0 | grep -q .; then
    echo "ERROR: replica $replica missing quantification result outputs" >&2
    exit 1
  fi
  if ! find "$outdir" -type f \( -name '*.idXML' -o -name '*.consensusXML' -o -name '*.featureXML' \) -size +0 | grep -q .; then
    echo "ERROR: replica $replica missing OpenMS intermediate outputs" >&2
    exit 1
  fi
  output_count="$(find "$outdir" -type f -size +0 | wc -l)"
  if [ "$output_count" -lt 50 ]; then
    echo "ERROR: replica $replica too few non-empty quantms outputs: $output_count" >&2
    exit 1
  fi
  find "$outdir" -type f -size +0 | sort > "$rep_dir/nonempty-outputs.txt"
  echo "nf-core/quantms replica $replica native Nextflow baseline completed with $output_count non-empty output files"
}
...
echo "nf-core/quantms native 4-node Nextflow baseline completed across 4 nodes"
