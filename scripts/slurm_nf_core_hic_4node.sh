...
#SBATCH --output=runs/nf-core_hic/slurm-%j-4node.out
#SBATCH --error=runs/nf-core_hic/slurm-%j-4node.err
...
if [ ! -s "$INPUT" ]; then
  echo "ERROR: missing Hi-C full samplesheet: $INPUT" >&2
  exit 1
fi
...
export NXF_OFFLINE=true
  ...
  if ! find "$outdir/contact_maps" -type f -size +0 | grep -q .; then
    echo "ERROR: replica $replica missing non-empty contact map outputs" >&2
    exit 1
  fi
  if ! find "$outdir/hicpro" -type f -size +0 | grep -q .; then
    echo "ERROR: replica $replica missing non-empty HiC-Pro outputs" >&2
    exit 1
  fi
  output_count="$(find "$outdir" -type f -size +0 | wc -l)"
  if [ "$output_count" -lt 50 ]; then
    echo "ERROR: replica $replica too few non-empty Hi-C outputs: $output_count" >&2
    exit 1
  fi
  find "$outdir" -type f -size +0 | sort > "$rep_dir/nonempty-outputs.txt"
  echo "nf-core/hic replica $replica native Nextflow baseline completed with $output_count non-empty output files"
}
...
echo "nf-core/hic native 4-node Nextflow baseline completed across 4 nodes"
