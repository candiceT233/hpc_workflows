...
#SBATCH --output=runs/nf-core_bacass/slurm-%j-4node-profiled.out
#SBATCH --error=runs/nf-core_bacass/slurm-%j-4node-profiled.err
...
RUN_ROOT="$ROOT/runs/nf-core_bacass"
...
for required in "$NF_ENV/bin/nextflow" "$DATALIFE_LIB" "$DARSHAN_LIB" "$DARSHAN_PARSER" "$ROOT/scripts/profile_tree_io.py" \
  "$INPUT_TSV" "$FASTQ_DIR/SRR8482585_1.fastq.gz" "$FASTQ_DIR/SRR8482585_2.fastq.gz" \
  "$FASTQ_DIR/SRR10093029_1.fastq.gz" "$FASTQ_DIR/SRR10093029_2.fastq.gz"; do
  if [ ! -s "$required" ] && [ ! -x "$required" ]; then
    echo "ERROR: missing required nf-core/bacass profiling artifact: $required" >&2
    exit 1
  fi
done
...
run_replica() {
  ...
  local override="$rep_dir/bacass_ares_override.config"
...
export NXF_OFFLINE=true
  ...
  if [ "$mode" = "datalife" ]; then
    env DATALIFE_OUTPUT_PATH="$trace_dir" \
      DATALIFE_FILE_PATTERNS="*.fastq.gz,*.fq.gz,*.fasta,*.fa,*.fna,*.gff,*.gff3,*.gbk,*.bam,*.bai,*.vcf,*.vcf.gz,*.tsv,*.csv,*.txt,*.html,*.json,*.log,*.yaml,*.yml" \
      LD_PRELOAD="$DATALIFE_LIB" \
      "$NF_ENV/bin/python" "$ROOT/scripts/profile_tree_io.py" \
        --root "$outdir" \
        --manifest "$rep_dir/profiled-output-digests.json" \
        --patterns '*.fastq.gz' '*.fq.gz' '*.fasta' '*.fa' '*.fna' '*.gff' '*.gff3' '*.gbk' '*.bam' '*.bai' '*.vcf' '*.vcf.gz' '*.tsv' '*.csv' '*.txt' '*.html' '*.json' '*.log' '*.yaml' '*.yml'
  ...
  echo "nf-core/bacass ${mode} replica $replica native Nextflow workflow completed with $output_count non-empty output files"
}
...
echo "nf-core/bacass native 4-node profiled Nextflow workflow completed across ${node_count} nodes"
