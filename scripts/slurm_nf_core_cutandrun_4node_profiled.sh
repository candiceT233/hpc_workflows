#!/usr/bin/env bash
#SBATCH --job-name=cutandrun-prof
#SBATCH --exclusive
...
#SBATCH --output=runs/nf-core_cutandrun/slurm-%j-4node-profiled.out
#SBATCH --error=runs/nf-core_cutandrun/slurm-%j-4node-profiled.err
...
REPO="$ROOT/repos/nf-core_cutandrun"
RUN_ROOT="$ROOT/runs/nf-core_cutandrun"
RUN_DIR="$RUN_ROOT/4node-profiled-${SLURM_JOB_ID:-manual}"
NF_ENV="$ROOT/tools/conda-envs/nextflow"
MINIFORGE="$ROOT/tools/miniforge"
source "$ROOT/scripts/cutandrun_full_inputs.sh"
...
for required in "$NF_ENV/bin/nextflow" "$DATALIFE_LIB" "$DARSHAN_LIB" "$DARSHAN_PARSER" "$ROOT/scripts/profile_tree_io.py"; do
  if [ ! -s "$required" ] && [ ! -x "$required" ]; then
    echo "ERROR: missing required nf-core/cutandrun profiling artifact: $required" >&2
    exit 1
  fi
done
...
cd "$ROOT"
prepare_cutandrun_full_inputs "$ROOT"
...
run_replica() {
  ...
  local override="$rep_dir/cutandrun_ares_override.config"
...
export NXF_OFFLINE=true
  ...
  nextflow run "$REPO" \
    ...
    -c "$override" \
    -work-dir "$workdir" \
    --input "$CUTANDRUN_SAMPLESHEET" \
    --genome GRCh38 \
    --igenomes_base "$CUTANDRUN_IGENOMES_BASE" \
  ...
  if [ "$mode" = "datalife" ]; then
    env DATALIFE_OUTPUT_PATH="$trace_dir" \
      DATALIFE_FILE_PATTERNS="*.fastq.gz,*.fq.gz,*.bam,*.bai,*.bed,*.bedgraph,*.bigWig,*.bw,*.narrowPeak,*.txt,*.tsv,*.csv,*.html,*.json,*.log,*.yaml,*.yml" \
      LD_PRELOAD="$DATALIFE_LIB" \
      "$NF_ENV/bin/python" "$ROOT/scripts/profile_tree_io.py" \
        --root "$outdir" \
        --manifest "$rep_dir/profiled-output-digests.json" \
        --patterns '*.fastq.gz' '*.fq.gz' '*.bam' '*.bai' '*.bed' '*.bedgraph' '*.bigWig' '*.bw' '*.narrowPeak' '*.txt' '*.tsv' '*.csv' '*.html' '*.json' '*.log' '*.yaml' '*.yml'
  ...
  echo "nf-core/cutandrun ${mode} replica $replica native Nextflow workflow completed with $output_count non-empty output files"
}
...
echo "nf-core/cutandrun native 4-node profiled Nextflow workflow completed across ${node_count} nodes"
