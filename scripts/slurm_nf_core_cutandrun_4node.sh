#!/usr/bin/env bash
#SBATCH --job-name=cutandrun-4node
#SBATCH --exclusive
...
#SBATCH --output=runs/nf-core_cutandrun/slurm-%j-4node.out
#SBATCH --error=runs/nf-core_cutandrun/slurm-%j-4node.err
...
export NXF_OFFLINE=true
  ...
  echo "nf-core/cutandrun replica $replica native Nextflow baseline completed with $output_count non-empty output files"
}
...
echo "nf-core/cutandrun native 4-node Nextflow baseline completed across 4 nodes"
