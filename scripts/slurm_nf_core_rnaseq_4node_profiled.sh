...
#SBATCH --output=runs/nf-core_rnaseq/slurm-%j-4node-profiled.out
#SBATCH --error=runs/nf-core_rnaseq/slurm-%j-4node-profiled.err
...
for required in "$NF_ENV/bin/nextflow" "$DATALIFE_LIB" "$DARSHAN_LIB" "$DARSHAN_PARSER" "$ROOT/scripts/profile_tree_io.py" "$REMOTE_INPUT"; do
  if [ ! -s "$required" ] && [ ! -x "$required" ]; then
    echo "ERROR: missing required nf-core/rnaseq profiling artifact: $required" >&2
    exit 1
  fi
done
...
validate_outputs() {
  ...
  if ! find "$outdir/star_salmon" -type f \( -name '*.bam' -o -name '*.bai' \) -size +0 | grep -q .; then
    echo "ERROR: $label missing non-empty STAR/BAM outputs" >&2
    exit 1
  fi
  if ! find "$outdir/star_salmon" -type f \( -name 'quant.sf' -o -name '*.counts.tsv' -o -name '*.featureCounts.txt' \) -size +0 | grep -q .; then
    echo "ERROR: $label missing non-empty quantification/count outputs" >&2
    exit 1
  fi
  output_count="$(find "$outdir" -type f -size +0 | wc -l)"
  if [ "$output_count" -lt 100 ]; then
    echo "ERROR: $label too few non-empty rnaseq outputs: $output_count" >&2
    exit 1
  fi
  echo "$output_count"
}
...
export NXF_OFFLINE=true
  ...
  if [ "$mode" = "datalife" ]; then
    env DATALIFE_OUTPUT_PATH="$trace_dir" \
      DATALIFE_FILE_PATTERNS="*.fastq.gz,*.fq.gz,*.bam,*.bai,*.sam,*.sf,*.gtf,*.fa,*.fai,*.txt,*.tsv,*.csv,*.html,*.log,*.pdf,*.png,*.gz" \
      LD_PRELOAD="$DATALIFE_LIB" \
      "$NF_ENV/bin/python" "$ROOT/scripts/profile_tree_io.py" \
        --root "$outdir" \
        --manifest "$rep_dir/profiled-output-digests.json" \
        --patterns '*.fastq.gz' '*.fq.gz' '*.bam' '*.bai' '*.sam' '*.sf' '*.gtf' '*.fa' '*.fai' '*.txt' '*.tsv' '*.csv' '*.html' '*.log' '*.pdf' '*.png' '*.gz'
  ...
  echo "nf-core/rnaseq ${mode} replica $replica native Nextflow workflow completed with $output_count non-empty output files"
}
...
echo "nf-core/rnaseq native 4-node profiled Nextflow workflow completed across ${node_count} nodes"
