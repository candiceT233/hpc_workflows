...
#SBATCH --output=runs/nf-core_hic/slurm-%j-4node-profiled.out
#SBATCH --error=runs/nf-core_hic/slurm-%j-4node-profiled.err
...
RUN_ROOT="$ROOT/runs/nf-core_hic"
...
for required in "$NF_ENV/bin/nextflow" "$DATALIFE_LIB" "$DARSHAN_LIB" "$DARSHAN_PARSER" "$ROOT/scripts/profile_tree_io.py" "$INPUT"; do
  if [ ! -s "$required" ] && [ ! -x "$required" ]; then
    echo "ERROR: missing required nf-core/hic profiling artifact: $required" >&2
    exit 1
  fi
done
...
run_replica() {
  ...
  local override="$rep_dir/hic_ares_override.config"
...
export NXF_OFFLINE=true
  ...
  if ! find "$outdir/contact_maps" -type f -size +0 | grep -q .; then
    echo "ERROR: ${mode} replica $replica missing non-empty contact map outputs" >&2
    exit 1
  fi
  if ! find "$outdir/hicpro" -type f -size +0 | grep -q .; then
    echo "ERROR: ${mode} replica $replica missing non-empty HiC-Pro outputs" >&2
    exit 1
  fi
  output_count="$(find "$outdir" -type f -size +0 | wc -l)"
  if [ "$output_count" -lt 50 ]; then
    echo "ERROR: ${mode} replica $replica too few non-empty Hi-C outputs: $output_count" >&2
    exit 1
  fi
  ...
  if [ "$mode" = "datalife" ]; then
    env DATALIFE_OUTPUT_PATH="$trace_dir" \
      DATALIFE_FILE_PATTERNS="*.fastq.gz,*.fq.gz,*.bam,*.bai,*.sam,*.pairs,*.cool,*.mcool,*.hic,*.matrix,*.bed,*.bedpe,*.tsv,*.csv,*.txt,*.html,*.json,*.log,*.pdf,*.png,*.gz" \
      LD_PRELOAD="$DATALIFE_LIB" \
      "$NF_ENV/bin/python" "$ROOT/scripts/profile_tree_io.py" \
        --root "$outdir" \
        --manifest "$rep_dir/profiled-output-digests.json" \
        --patterns '*.fastq.gz' '*.fq.gz' '*.bam' '*.bai' '*.sam' '*.pairs' '*.cool' '*.mcool' '*.hic' '*.matrix' '*.bed' '*.bedpe' '*.tsv' '*.csv' '*.txt' '*.html' '*.json' '*.log' '*.pdf' '*.png' '*.gz'
  ...
  echo "nf-core/hic ${mode} replica $replica native Nextflow workflow completed with $output_count non-empty output files"
}
...
echo "nf-core/hic native 4-node profiled Nextflow workflow completed across ${node_count} nodes"
