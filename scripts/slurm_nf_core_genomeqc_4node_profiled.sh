...
#SBATCH --output=runs/nf-core_genomeqc/slurm-%j-4node-profiled.out
#SBATCH --error=runs/nf-core_genomeqc/slurm-%j-4node-profiled.err
...
for required in "$NF_ENV/bin/nextflow" "$DATALIFE_LIB" "$DARSHAN_LIB" "$DARSHAN_PARSER" "$ROOT/scripts/profile_tree_io.py" "$INPUT"; do
  if [ ! -s "$required" ] && [ ! -x "$required" ]; then
    echo "ERROR: missing required nf-core/genomeqc profiling artifact: $required" >&2
    exit 1
  fi
done
...
export NXF_OFFLINE=true
  ...
  if ! find "$outdir/quast" -type f -size +0 | grep -q .; then
    echo "ERROR: ${mode} replica $replica missing non-empty QUAST outputs" >&2
    exit 1
  fi
  if ! find "$outdir/busco" -type f -size +0 | grep -q .; then
    echo "ERROR: ${mode} replica $replica missing non-empty BUSCO outputs" >&2
    exit 1
  fi
  output_count="$(find "$outdir" -type f -size +0 | wc -l)"
  if [ "$output_count" -lt 30 ]; then
    echo "ERROR: ${mode} replica $replica too few non-empty GenomeQC outputs: $output_count" >&2
    exit 1
  fi
  ...
  if [ "$mode" = "datalife" ]; then
    env DATALIFE_OUTPUT_PATH="$trace_dir" \
      DATALIFE_FILE_PATTERNS="*.fa,*.fasta,*.fna,*.gff,*.gff3,*.gbff,*.tsv,*.csv,*.txt,*.html,*.json,*.log,*.yaml,*.yml,*.gz" \
      LD_PRELOAD="$DATALIFE_LIB" \
      "$NF_ENV/bin/python" "$ROOT/scripts/profile_tree_io.py" \
        --root "$outdir" \
        --manifest "$rep_dir/profiled-output-digests.json" \
        --patterns '*.fa' '*.fasta' '*.fna' '*.gff' '*.gff3' '*.gbff' '*.tsv' '*.csv' '*.txt' '*.html' '*.json' '*.log' '*.yaml' '*.yml' '*.gz'
  ...
  echo "nf-core/genomeqc ${mode} replica $replica native Nextflow workflow completed with $output_count non-empty output files"
}
...
echo "nf-core/genomeqc native 4-node profiled Nextflow workflow completed across ${node_count} nodes"
