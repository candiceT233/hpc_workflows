...
#SBATCH --output=runs/nf-core_taxprofiler/slurm-%j-4node-profiled.out
#SBATCH --error=runs/nf-core_taxprofiler/slurm-%j-4node-profiled.err
...
for required in "$NF_ENV/bin/nextflow" "$DATALIFE_LIB" "$DARSHAN_LIB" "$DARSHAN_PARSER" "$ROOT/scripts/profile_tree_io.py"; do
  if [ ! -s "$required" ] && [ ! -x "$required" ]; then
    echo "ERROR: missing required nf-core/taxprofiler profiling artifact: $required" >&2
    exit 1
  fi
done
...
EOF
  ...
  cd "$rep_dir"
  nextflow run "$REPO" \
    -name "taxprofiler_${mode}_${SLURM_JOB_ID:-manual}_${replica}" \
    -profile conda \
  ...
  if [ "$mode" = "datalife" ]; then
    env DATALIFE_OUTPUT_PATH="$trace_dir" \
      DATALIFE_FILE_PATTERNS="*.fastq.gz,*.fq.gz,*.bam,*.bai,*.fastq,*.fq,*.fasta,*.fa,*.profile,*.tre,*.tsv,*.csv,*.txt,*.html,*.log,*.gz" \
      LD_PRELOAD="$DATALIFE_LIB" \
      "$NF_ENV/bin/python" "$ROOT/scripts/profile_tree_io.py" \
        --root "$outdir" \
        --manifest "$rep_dir/profiled-output-digests.json" \
        --patterns '*.fastq.gz' '*.fq.gz' '*.bam' '*.bai' '*.fastq' '*.fq' '*.fasta' '*.fa' '*.profile' '*.tre' '*.tsv' '*.csv' '*.txt' '*.html' '*.log' '*.gz'
  ...
  echo "nf-core/taxprofiler ${mode} replica $replica native Nextflow workflow completed with $output_count non-empty output files"
}
...
echo "nf-core/taxprofiler native 4-node profiled Nextflow workflow completed across ${node_count} nodes"
