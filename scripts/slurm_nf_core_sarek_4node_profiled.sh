...
#SBATCH --output=runs/nf-core_sarek/slurm-%j-4node-profiled.out
#SBATCH --error=runs/nf-core_sarek/slurm-%j-4node-profiled.err
...
RUN_ROOT="$ROOT/runs/nf-core_sarek"
...
for required in "$NF_ENV/bin/nextflow" "$DATALIFE_LIB" "$DARSHAN_LIB" "$DARSHAN_PARSER" "$ROOT/scripts/profile_tree_io.py"; do
  if [ ! -s "$required" ] && [ ! -x "$required" ]; then
    echo "ERROR: missing required nf-core/sarek profiling artifact: $required" >&2
    exit 1
  fi
done
...
validate_outputs() {
  ...
  if ! find "$outdir" -type f \( -name '*.bam' -o -name '*.bai' -o -name '*.cram' -o -name '*.crai' \) -size +0 | grep -q .; then
    echo "ERROR: $label missing non-empty alignment outputs" >&2
    exit 1
  fi
  if ! find "$outdir" -type f \( -name '*.vcf.gz' -o -name '*.bcf' \) -size +0 | grep -q .; then
    echo "ERROR: $label missing non-empty variant outputs" >&2
    exit 1
  fi
  output_count="$(find "$outdir" -type f -size +0 | wc -l)"
  if [ "$output_count" -lt 100 ]; then
    echo "ERROR: $label too few non-empty Sarek outputs: $output_count" >&2
    exit 1
  fi
  echo "$output_count"
}
...
run_replica() {
  ...
  local override="$rep_dir/sarek_ares_override.config"
...
export NXF_OFFLINE=true
  ...
  if [ "$mode" = "datalife" ]; then
    env DATALIFE_OUTPUT_PATH="$trace_dir" \
      DATALIFE_FILE_PATTERNS="*.fastq.gz,*.fq.gz,*.bam,*.bai,*.cram,*.crai,*.vcf.gz,*.bcf,*.tbi,*.bed,*.txt,*.tsv,*.csv,*.html,*.log,*.pdf,*.png,*.zip,*.gz" \
      LD_PRELOAD="$DATALIFE_LIB" \
      "$NF_ENV/bin/python" "$ROOT/scripts/profile_tree_io.py" \
        --root "$outdir" \
        --manifest "$rep_dir/profiled-output-digests.json" \
        --patterns '*.fastq.gz' '*.fq.gz' '*.bam' '*.bai' '*.cram' '*.crai' '*.vcf.gz' '*.bcf' '*.tbi' '*.bed' '*.txt' '*.tsv' '*.csv' '*.html' '*.log' '*.pdf' '*.png' '*.zip' '*.gz'
  ...
  echo "nf-core/sarek ${mode} replica $replica native Nextflow workflow completed with $output_count non-empty output files"
}
...
echo "nf-core/sarek native 4-node profiled Nextflow workflow completed across ${node_count} nodes"
