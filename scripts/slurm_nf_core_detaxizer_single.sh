#!/bin/bash
#SBATCH --job-name=detaxizer-single
#SBATCH --exclusive
...
#SBATCH --output=runs/nf-core_detaxizer/slurm-%j-single.out
#SBATCH --error=runs/nf-core_detaxizer/slurm-%j-single.err
...
OUTDIR="$ROOT/runs/nf-core_detaxizer/single-${SLURM_JOB_ID:-manual}/results"
WORKDIR="$ROOT/runs/nf-core_detaxizer/single-${SLURM_JOB_ID:-manual}/work"
RUN_DIR="$(dirname "$OUTDIR")"
NF_ENV="$ROOT/tools/conda-envs/nextflow"
MINIFORGE="$ROOT/tools/miniforge"

mkdir -p "$ROOT/runs/nf-core_detaxizer" "$RUN_DIR" "$OUTDIR" "$WORKDIR" "$ROOT/tools/nextflow-home" "$FASTQ_DIR" "$REF_DIR"
...
cat > "$RUN_DIR/detaxizer_ares_override.config" <<EOF
conda {
  enabled = true
  useMamba = true
...
nextflow run "$REPO" \
  ...
  -c "$RUN_DIR/detaxizer_ares_override.config" \
  -work-dir "$WORKDIR" \
  --input "$INPUT" \
...
if ! find "$OUTDIR/filter" -type f -name '*filtered*.fastq.gz' -size +0 | grep -q .; then
  echo "ERROR: missing filtered FASTQ outputs" >&2
  exit 1
fi

OUTPUT_COUNT="$(find "$OUTDIR" -type f -size +0 | wc -l)"
if [ "$OUTPUT_COUNT" -lt 20 ]; then
  echo "ERROR: too few non-empty outputs for full detaxizer baseline: $OUTPUT_COUNT" >&2
  exit 1
fi

echo "nf-core/detaxizer single-node native Nextflow baseline completed with $OUTPUT_COUNT non-empty output files"
