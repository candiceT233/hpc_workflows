...
#SBATCH --output=runs/nf-core_clipseq/slurm-%j-4node-profiled.out
#SBATCH --error=runs/nf-core_clipseq/slurm-%j-4node-profiled.err
...
for required in "$NF_ENV/bin/nextflow" "$DATALIFE_LIB" "$DARSHAN_LIB" "$DARSHAN_PARSER" "$ROOT/scripts/profile_tree_io.py" "$INPUT" "$FASTA" "$GTF"; do
  if [ ! -s "$required" ] && [ ! -x "$required" ]; then
    echo "ERROR: missing required nf-core/clipseq profiling artifact: $required" >&2
    exit 1
  fi
done
...
run_replica() {
  ...
  local podman_base="/tmp/pdm-clipseq-${SLURM_JOB_ID:-manual}-${mode}-${replica}"
...
EOF
  ...
  if [ "$mode" = "datalife" ]; then
    env DATALIFE_OUTPUT_PATH="$trace_dir" \
      DATALIFE_FILE_PATTERNS="*.fastq.gz,*.fq.gz,*.bam,*.bai,*.bed,*.bw,*.bigWig,*.txt,*.tsv,*.csv,*.html,*.json,*.log,*.yaml,*.yml" \
      LD_PRELOAD="$DATALIFE_LIB" \
      "$NF_ENV/bin/python" "$ROOT/scripts/profile_tree_io.py" \
        --root "$outdir" \
        --manifest "$rep_dir/profiled-output-digests.json" \
        --patterns '*.fastq.gz' '*.fq.gz' '*.bam' '*.bai' '*.bed' '*.bw' '*.bigWig' '*.txt' '*.tsv' '*.csv' '*.html' '*.json' '*.log' '*.yaml' '*.yml'
  ...
  echo "nf-core/clipseq ${mode} replica $replica native Nextflow workflow completed with $output_count non-empty output files"
}
...
echo "nf-core/clipseq native 4-node profiled Nextflow workflow completed across ${node_count} nodes"
