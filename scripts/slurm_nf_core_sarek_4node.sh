...
#SBATCH --output=runs/nf-core_sarek/slurm-%j-4node.out
#SBATCH --error=runs/nf-core_sarek/slurm-%j-4node.err
...
if [ ! -s "$LOCAL_INPUT" ]; then
  echo "ERROR: missing localized Sarek full samplesheet: $LOCAL_INPUT" >&2
  exit 1
fi
...
export NXF_OFFLINE=true
  ...
  echo "nf-core/sarek replica $replica native Nextflow baseline completed with $output_count non-empty output files"
}
...
echo "nf-core/sarek native 4-node Nextflow baseline completed across 4 nodes"
