#!/usr/bin/env bash
#SBATCH --job-name=pathosurv-prof
#SBATCH --exclusive
...
#SBATCH --output=runs/nf-core_pathogensurveillance/slurm-%j-4node-profiled.out
#SBATCH --error=runs/nf-core_pathogensurveillance/slurm-%j-4node-profiled.err
...
for required in "$NF_ENV/bin/nextflow" "$DATALIFE_LIB" "$DARSHAN_LIB" "$DARSHAN_PARSER" "$ROOT/scripts/profile_tree_io.py" "$SAMPLESHEET"; do
  if [ ! -s "$required" ] && [ ! -x "$required" ]; then
    echo "ERROR: missing required nf-core/pathogensurveillance profiling artifact: $required" >&2
    exit 1
  fi
done
...
export NXF_OFFLINE=true
  ...
  if ! find "$outdir/reports" -type f -name '*_pathsurveil_report.html' -size +100000 | grep -q .; then
    echo "ERROR: ${mode} replica $replica missing non-trivial pathogensurveillance HTML report" >&2
    exit 1
  fi
  ...
  if ! find "$outdir/quality_control" -type f \( -name '*.html' -o -name '*.json' -o -name 'report.tsv' \) -size +0 | grep -q .; then
    echo "ERROR: ${mode} replica $replica missing non-empty quality-control outputs" >&2
    exit 1
  fi
  output_count="$(find "$outdir" -type f -size +0 | wc -l)"
  if [ "$output_count" -lt 50 ]; then
    echo "ERROR: ${mode} replica $replica too few non-empty pathogensurveillance outputs: $output_count" >&2
    exit 1
  fi
  ...
  if [ "$mode" = "datalife" ]; then
    env DATALIFE_OUTPUT_PATH="$trace_dir" \
      DATALIFE_FILE_PATTERNS="*.fastq.gz,*.fq.gz,*.fasta,*.fa,*.fna,*.tsv,*.csv,*.txt,*.html,*.json,*.log,*.pdf,*.png,*.gz" \
      LD_PRELOAD="$DATALIFE_LIB" \
      "$NF_ENV/bin/python" "$ROOT/scripts/profile_tree_io.py" \
        --root "$outdir" \
        --manifest "$rep_dir/profiled-output-digests.json" \
        --patterns '*.fastq.gz' '*.fq.gz' '*.fasta' '*.fa' '*.fna' '*.tsv' '*.csv' '*.txt' '*.html' '*.json' '*.log' '*.pdf' '*.png' '*.gz'
  ...
  echo "nf-core/pathogensurveillance ${mode} replica $replica native Nextflow workflow completed with $output_count non-empty output files"
}
...
echo "nf-core/pathogensurveillance native 4-node profiled Nextflow workflow completed across ${node_count} nodes"
